...
void cxl_doe_cdat_init(CXLComponentState *cxl_cstate, Error **errp)
{
    ...
    if (fp) {
        ...
        while (i < file_size) {
            ...
            while (cdat->buf + i <
                   (char *)cdat_st[ent].base + cdat_st[ent].length) {
                assert(i < file_size);
                sum += cdat->buf[i++];
            }
        ...
        if (sum != 0) {
            warn_report("Found checksum mismatch in %s", cdat->filename);
        }
    } else {
        /* Use default table if fopen == NULL */
        assert(cdat->build_cdat_table);
        cdat_header = g_malloc0(sizeof(struct cdat_table_header));
        cdat->build_cdat_table(&cdat->built_buf, &cdat->built_buf_len);
        /* Entry 0 for CDAT header, starts with Entry 1 */
        for (; ent < cdat->built_buf_len + 1; ent++) {
            hdr = cdat->built_buf[ent - 1];
            buf = cdat->built_buf[ent - 1];
